...
var privateKey = fs.readFileSync('./keys/rsa_private.key');
var publicKey = fs.readFileSync('./keys/rsa_public.key');
 ...
 * @returns Promise
 */
async function create_session(user, req, {exp='12h', newSessionId=true}={}){
   ...
   let confirmed = false;
   let active = false;
   if (user.setup.confirmed_email && user.setup.confirmed_terms_conditions && user.setup.basic_info_passed){
      confirmed = true;
   ...
   let profile = {
      ...
      profile_image: {Location: _.get(user, 'profile_image.Location', undefined)},
...
async function refresh_token_info(req, res) {
   try {
      ...
      let session = await create_session(user, req, {exp:timeLeft.toString()+'ms'});
...
async function logout(req, res) {
   try {
      ...
      await req.my_db.models['session'].collection.findOneAndDelete({session_id})
...
async function login(req, res) {
   try {
      ...
      if (error_msg !== null) {
         ...
         req.log.warn(`User attempted failed login with email=${body.email}, pass=${pass}`)
...
async function confirmEmail(req, res) {
   try {
      ...
      if (user.setup.email_code !== body.code && user.setup.confirmed_email===false){
         res.status(403);
      ...
      //create session for user (auto log them in)
         ...
         if (user.setup.confirmed_email===true){
            ...
            res.send({msg: 'EmailConfirmed', jwt_token: session.jwt_token});
         }
...
async function confirmEmailChange(req, res) {
   try {
      ...
      if (user.email_change == null || user.email_change.confirm_code !== body.code){
         res.status(403);
...
async function create_user_db_record(email, pass, db, {name_first, name_last, forceReset=false, invited=false,
                                    acct_type}={}){
   ...
   let user = {
      ...
      setup: {email_code: confirm_code, invited_user: invited, pw_reset_code: pw_reset},
...
async function create_user (req, res) {
   try {
      ...
      var doc = await create_user_db_record(email, pass, req.my_db, {acct_type: body.account_type, forceReset:true})


      //step 3: send email to user******************************************************************
      let email_code = doc.setup.email_code;
      let pw_code = doc.setup.pw_reset_code;
      let email_res = await emailer.send_email_confirmation({to: email,  email_code, pw_code});
