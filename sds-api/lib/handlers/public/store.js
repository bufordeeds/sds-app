...
function initializeConfigurations() {
	console.log('Loaded EasyPost API Key:', process.env.EASYPOST_API_KEY);

	if (!process.env.EASYPOST_API_KEY) {
		console.error('EasyPost API Key not found in environment variables!');
		process.exit(1);
	}
...
async function createCheckoutSession(req, res) {
	try {
		...
		let cart = await req.my_db.models['cart'].collection.findOne({
		...
		await helpers.update_db_record(cart_update, req.my_db.models['cart'], {
			validationParams
		});
...
async function createCheckoutSessionDonation(req, res) {
	try {
		...
		let error_msg = helpers.check_params2(body, params, {
			required: ['donation_amount', 'back_url'],
			res
		});
		if (error_msg !== null) {
			return;
...
async function updateCartItems(req, res) {
	try {
		...
		let c2 = await helpers.update_db_record(update, mld_cart, {
			validationParams
		});
...
async function closeCartAndCreateOrder(req, res) {
	try {
		...
		//*************create order*****************************
		let stripe_session = await stripe.checkout.sessions.retrieve(
		...
		let donation;
		if (
			stripe_session.client_reference_id != null &&
			stripe_session.client_reference_id.includes('cart')
		) {
			//*************Close Cart*****************************
			let mld_cart = req.my_db.models['cart'];

			let cart = await mld_cart.collection.findOne({
			...
			stripe_session.client_reference_id != null &&
			stripe_session.client_reference_id.includes('userid')
		) {
			...
			if (id !== 'guest') {
				...
				donation = await store.create_donation({
...
async function getMyOrders(req, res) {
	try {
		...
		//check if user has active cart
		let orders = await req.my_db.models['order'].collection
			.find({ user_id }, { sort: { date_ordered: -1 } })
			.toArray();
...
async function verifyAddress(req, res) {
	try {
		...
		error_msg = helpers.check_params2(body.address, params, {
...
async function createShippingLabel(req, res) {
	try {
		...
		let sds_info = await req.my_db.models['sds_info'].collection.findOne({
			req_name: 'SDS_COMPANY_INFO'
		});
		...
		if (from_ep_id == undefined) {
			...
			let validationParams = { required: [], additionalProperties: true };
			await helpers.update_db_record(
				update,
				req.my_db.models['sds_info'],
				{ validationParams }
			);
		...
		let validationParams = { required: [], additionalProperties: true };
		await helpers.update_db_record(cart_update, mld_cart, {
			validationParams
		});
...
async function getItem(req, res) {
	try {
		...
		let item = await mld_item.collection.findOne({
