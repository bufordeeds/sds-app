  ...
  <div>
    ...
    <v-dialog v-model="show_edit_user" persistent>
      ...
      <div class="content-container-bg" style="background-color: var(--color-bg)">
        <edit-trainer-info v-if="selected_user && selected_user.account_type === 'TRAINER'" :user_id="selected_user._id"
          @user_updated="on_record_update" @photo_updated="on_record_update({ close: false })" />


        <edit-admin-info v-if="selected_user && selected_user.account_type === 'SDS-ADMIN'" :user_id="selected_user._id"
          @user_updated="on_record_update" @photo_updated="on_record_update({ close: false })" />

        <div class="content-container-sm">
          <edit-handler-info v-if="selected_user && selected_user.account_type === 'HANDLER'"
            :user_id="selected_user._id" @user_updated="on_record_update"
    ...
    <v-dialog v-model="show_edit_dog" persistent max-width="400px">
      ...
      <edit-dog-dialog v-if="show_edit_dog" :dog="selected_dog" @update="on_record_update"
        @update-image="on_record_update({ close: false })" @close="show_edit_dog = false;" />
    </v-dialog>
    ...
    <v-dialog v-model="show_edit_comments" persistent max-width="700px">
      <v-card v-if="show_edit_comments" class="pa-3">
        ...
        <my-form>
          <my-text-area v-model="selected_user.admin.comment" label="Comments" />
        </my-form>
    ...
    <v-dialog v-model="show_edit_comments_dog" persistent max-width="700px">
      <v-card v-if="show_edit_comments_dog" class="pa-3">
        <div class="dialog-heading" style="display: flex">
          Admin Comments
          <v-spacer />
        ...
        <my-form>
          <my-text-area v-model="selected_dog.admin.comment" label="Comments" />
        </my-form>
    ...
    <div class="content-container-bg">
      <div style="display: flex; width: 100%" class="pl-0 pt-4">
        <my-date-picker v-model="date_start" style="width: 150px; background-color: white" label="Date From" readonly />

        <my-date-picker v-model="date_end" class="ml-4" style="width: 150px; background-color: white" label="Date To"
          readonly />
      </div>
      ...
      <my-form ref="form">
        <v-row dense>
          <v-col>
            <my-drop-down v-model="user_type" label="Search Type" :list="type_list" item-text="txt" item-value="val" />
          </v-col>

          <v-col>
            <my-text-input v-model="name_first" label="First Name" clearable @keyup-enter="search" />
          </v-col>

          <v-col>
            <my-text-input v-model="name_last" label="Last Name" clearable @keyup-enter="search" />
          </v-col>


          <v-col>
            <my-text-input v-model="user_city" label="City" clearable @keyup-enter="search" />
          </v-col>


          <v-col>
            <my-text-input v-model="user_state" label="State" clearable @keyup-enter="search" />
          </v-col>

          <v-col v-if="!isDog">
            <my-text-input v-model="email" label="Email" clearable @keyup-enter="search" />
          </v-col>



          <v-col v-if="isDog">
            <my-text-input v-model="dog_name" label="Dog's Name" clearable @keyup-enter="search" />
          </v-col>

          <v-col v-if="isDog">
            <my-text-input v-model="sds_num" label="SDS Number" clearable />
          </v-col>

          <v-btn class="mt-6" @click="search">
            Search
          </v-btn>
        </v-row>
      ...
      <div class="content-container mt-8" style="background-color: white">
        <table-dogs v-if="isDog" :dogs="dogs" @edit-comment="on_edit_comment_dog" @click-user="on_click_dog_user"
          @click-dog="on_click_dog" />

        <table-users v-else :users="users" @click-user="on_click_user" @edit-comment="on_edit_comment_user" />
      </div>
    </div>
  </div>
</template>
...
export default {
  name: "AdminUsers",
  components: {
    MyDropDown, myDatePicker, tableUsers, tableDogs, EditHandlerInfo, EditTrainerInfo, EditAdminInfo,
